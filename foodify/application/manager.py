...
class FoodifyManager:
    def add_product(self, db, barcode: str, recurrent: bool, units: int):
        ...
        product_added = Products.get_or_create_product(barcode=barcode, recurrent=recurrent, units=units, db=db)

        product_in_fridge = db.query(Fridge).filter(Fridge.product_id == product_added.id,
        ...
        if not product_in_fridge:
            ...
            unit_packaging = db.query(Products.unit_packaging).filter(Products.id == product_added.id).first()
            new_unit_actual = unit_actual + unit_packaging[0]
            # Actualiza las unit_actual al nuevo valor actual
            db.query(Fridge).filter(Fridge.product_id == product_added.id, Fridge.date_out is None) \
    ...
    def add_super_first_time(self, db, product_added):
        ...
        super_list = []
        relation = db.query(ProductSuperRelationship.super_id).filter(
        ...
        if not super_list:
            ...
            Supermarket.extract_prices_supermarkets(ean=product_added.ean, product_added=product_added, db=db)
    ...
    def new_product(self, db, detected_barcodes: tuple, recurrent: bool, units: int):
        ...
        for barcode in detected_barcodes:
            ...
            product, info = self.add_product(barcode=barcode_to_use, recurrent=recurrent, units=units, db=db)
    ...
    def old_product(self, db, detected_barcodes: tuple):
        ...
        for barcode in detected_barcodes:
            # Devuelve el código de barras
            barcode_to_use = barcode.data.decode("utf-8")
            product, info = self.check_unit_actual_in_fridge(barcode_to_use=barcode_to_use, db=db)
    ...
    def check_unit_actual_in_fridge(self, db, barcode_to_use: str):
        ...
        try:
            ...
            product_fridge = db.query(Fridge).filter(Fridge.product_id == product.id).order_by(
            ...
            if new_unit_actual == 0:
                ...
                if product.recurrent:
                    ...
                    check = db.query(ProductSuperRelationship) \
                    ...
                    if check:
                        ...
                        ShoppingList.send_to_shopping_list(product_fridge=product_fridge, db=db)
                        info = f'El producto {product.name} se ha añadido a la lista de la compra'
                        return product, info
                    else:
                        Supermarket.extract_prices_supermarkets(ean=product.ean, product_added=product, db=db)
                        ShoppingList.send_to_shopping_list(product_fridge=product_fridge, db=db)
                        info = f'El producto {product.name} se ha añadido a la lista de la compra'
                        return product, info
    ...
    def buy_shopping_list(self, db):
        ...
        for product in buy_products:
            ...
            product_name = db.query(Products.name).filter(Products.id == product.product_id).first()

            # Traigo el nombre del supermercado de Supermarket
            super_name = db.query(Supermarket.name).filter(Supermarket.id == product.super_id).first()
            ...
            # Actualiza las unit_actual al nuevo valor actual
            db.query(ShoppingList).filter(ShoppingList.product_id == product.product_id,
            ...
            product_added = db.query(Products).filter(Products.id == product.product_id).first()
            Fridge.save_fridge_product(product_added=product_added, db=db)
