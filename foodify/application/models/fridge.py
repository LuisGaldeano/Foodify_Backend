...
class Fridge(Base):
    ...
    def save_fridge_product(cls, db, product_added: object) -> None:
        try:
            ...
            fridge_entry = Fridge(
                product_id=product_added.id, unit_actual=new_unit_actual
            )
    ...
    def get_fridge_products(cls, db):
        ...
        for product_in_fridge in all_products_in_fridge:
            name = (
                ...
                .filter(Products.id == product_in_fridge.product_id)
                .first()
            )
    ...
    @classmethod
    def update_fridge_products(
        cls, db, old_product_data: str, new_product_data: str
    ) -> str:
        """
        ...
        """
        product_to_update = (
            db.query(Products).filter(Products.name == old_product_data).first()
        )
        if product_to_update:
            try:
                ...
                logger.info(f"The following exception occurred: {e}")
                return (
                    f"Supermarket '{old_product_data}' has not been successfully updated."
                )
        else:
    ...
    def delete_fridge_product(cls, db, product_data: str) -> str:
        ...
        """
        product_to_delete = (
            db.query(Supermarket).filter(Supermarket.name == product_data).first()
        )
